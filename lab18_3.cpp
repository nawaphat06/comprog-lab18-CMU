...
student text2student(string text){
    ...
    s.id = stoi(id.c_str());
    s.gender = gen[0];
    s.gpa = stof(gpa.c_str());
    
    return s;
}


student * findstudent(vector<student> &allstudents,int key){ //[Missing Code 4] There is something wrong in this line. (Added '&' for pass-by-reference)
